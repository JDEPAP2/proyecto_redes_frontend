...
export default function CotizacionItem({ eventId, cotizacionId, cantidad, estado, onEdit }) {
    ...
    const estadoColor =
        estadoClean === "pendiente" ? "bg-yellow-400" :
        (estadoClean === "aceptada" || estadoClean === "aprobado") ? "bg-green-600" :
        estadoClean === "rechazada" ? "bg-red-600" : "bg-gray-400";
            ...
            <div className="flex flex-col w-full space-y-2">
                <p className="text-slate-400 font-bold text-lg">
                    {usuario} | <span className="text-black text-md font-semibold">
                        {event?.ubicacion || "Ubicación no especificada"}</span>
                </p>
